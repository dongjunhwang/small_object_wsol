...
class InceptionCam(nn.Module):
    def __init__(self, num_classes=1000, large_feature_map=False, **kwargs):
        ...
        self.use_inception_c = kwargs['use_inception_c']
    ...
    def forward(self, x, labels=None, return_cam=False, crop=False, da=False):
        ...
        if return_cam:
            if self.use_inception_c:
                feature_map = feat.clone().detach()
            else:
        ...
        if crop:
            feature_map = feat if self.use_inception_c else feat_map
            return {'cam_weights': None,
                    'logits': logits, 'feature_map': feature_map}

        if da:
            feature_map = feat if self.use_inception_c else feat_map
            return {'pixel_feature': x, 'image_feature': self.avgpool(x),
                    'logits': logits,
                    'cam_weights': None, 'feature_map': feature_map}
...
class InceptionAcol(AcolBase):
    ...
    def forward(self, x, labels=None, return_cam=False):
        ...
        if return_cam:
            ...
            if self.use_inception_c:
                feature_maps = feature.clone().detach()
            else:
                feature_maps = torch.max(normalized_a, normalized_b)
            if self.attention_cam:
                cams = feature_maps.mean(1, keepdim=False)
            else:
...
class InceptionAdl(nn.Module):
    ...
    def forward(self, x, labels=None, return_cam=False):
        ...
        x = self.Mixed_6d(x)
        x = self.Mixed_6e(x)
        if self.use_inception_c and self.attention_cam and return_cam:
        ...
        if return_cam:
            if self.use_inception_c and self.attention_cam:
                cams = feature_map.mean(1, keepdim=False)
            else:
                feature_map = x.clone().detach()
                cams = feature_map[range(batch_size), labels]
